...
# Функция в которой задаётся разметка клавиатуры
def get_on_start_kb():
    button_start = KeyboardButton(text = "Start")
    button_help = KeyboardButton(text = "Help")
    button_wallet = KeyboardButton(text = "Wallet/balance")
    button_stop = KeyboardButton(text = "Stop")
    buttons_row = [button_start, button_stop,button_wallet, button_help]
...
# Функция START - начало работы бота
#
# активирует клавиатуру
# пишет приветствие
# активирует алгоритм поиска сделок
...
async def handle_start(message: types.Message):
    ...
    while True:
        ...
        data_income = get_income(data_prices, data_chains[i], usdt)
        global s
        s = str(0) + " " + str(data_chains[i]) + " " + str(data_income)
        await message.answer(
            text = s
        )
        i += 1
...
# Функция STOP - останавливает цикл работы бота
...
# Функция Help - вызывает обработчик сообщения help
@dp.message(F.text == "Help")
@dp.message(Command("help"))
async def handle_help(message: types.Message):
    text = "Я криптовалютный бот для арбитража который поможет тебе совершать сделки на платформе bitget.\nОтправь мне любое сообщение."
